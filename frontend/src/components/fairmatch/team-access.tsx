...
export function JoinOrganization({ onSession, onBack }: { onSession: (token: string) => void; onBack: () => void }) {
  ...
  return <main className="fm-login" id="main-content">
    ...
    <form onSubmit={async e => {
      ...
      <Field label="Choose an employer username" hint="Start with a letter; use 3–60 letters, numbers, dots, underscores or hyphens."><Input required value={form.username} onChange={e => setForm({ ...form, username: e.target.value })} minLength={3} maxLength={60} pattern="[A-Za-z][A-Za-z0-9._-]{2,59}" title="Start with a letter and use 3–60 letters, numbers, dots, underscores or hyphens." autoComplete="username" disabled={busy} /></Field>
