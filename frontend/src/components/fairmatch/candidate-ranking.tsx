...
function submittedSkills(source:string,candidate:Candidate|undefined,excerpt:string) {
 if(source==="skills"&&candidate?.skills?.length)return candidate.skills;
 if(source==="cv_skills"&&candidate?.cvSummary?.skills?.length)return candidate.cvSummary.skills;
 if((source==="skills"||source==="cv_skills")&&excerpt)return excerpt.split(/\s*,\s*/g).filter(Boolean);
 return [];
}
function containsMatchedWord(value:string,words:string[]) {
 const terms:string[]=value.toLocaleLowerCase().match(/[\p{L}\p{N}]+/gu)??[];
 return words.some(word=>terms.includes(word.toLocaleLowerCase()));
}
export function CandidateRanking({jobs,candidates,auth,initialJobId,onCandidateAction}:{jobs:Job[];candidates:Candidate[];auth:string;initialJobId?:string;onCandidateAction:(action:CandidateAction,candidate:Candidate)=>void}) {
 ...
 return <>
  <PageHeading eyebrow="EXPLAINED COMPARISON" title="Automatic candidate ranking" description="Compare applications using job requirements, then open any candidate to review evidence or continue the hiring process." actions={<Button variant="outline" onClick={refresh}><RefreshCw size={16}/>Refresh ranking</Button>}/>
  <section className="fs-ranking-intro" aria-label="How automatic ranking works"><Sparkles size={20}/><div><strong>How the automatic score works</strong><p>FairMatch finds job-requirement words in each candidate&apos;s submitted experience, skills and shared CV highlights. A higher score means more requirement words were found. Always read the cited passages before making a hiring decision.</p></div></section>
   ...
   <div className="fs-ranking-workspace">
    <Panel className="fs-ranking-results" title="Ranked candidates" description={`${current.jobTitle} · ${stage} · ${current.rubricCurrent?"Current rubric weights":"Equal requirement weights"}`}>
     ...
     <div className="fs-ranking-list">{current.autoRanked.map(row=><article className="fs-ranking-candidate" key={row.applicationId}>
      <div className="fs-ranking-candidate-main"><span className="fs-ranking-position" aria-label={`Rank ${row.rank}`}>{row.rank}</span><div className="fs-ranking-identity"><button onClick={()=>openCandidate(row.applicationId)}>{row.applicationId}<ArrowUpRight size={15}/></button><p>{row.matched} of {row.total} requirements contain a keyword match</p></div><div className="fs-ranking-score"><strong>{row.score.toFixed(0)}</strong><span>/ 100 keyword coverage</span></div></div>
      ...
      <details className="fs-ranking-details"><summary>Why this position?<ChevronRight size={16}/></summary>{row.matches.map((match,index)=>{const skills=submittedSkills(match.source,candidates.find(candidate=>candidate.id===row.applicationId),match.excerpt);return <section key={index}><div className="fs-ranking-match-heading"><h3>{match.requirement} · {match.weight}% of total score</h3><span>Keyword coverage</span></div><p><strong>Why it received points:</strong> {match.matchedWords.length}/{match.totalTerms} meaningful requirement terms were found in one submitted passage. This awarded {match.points.toFixed(2)} of the criterion&apos;s {match.weight} available text-match points.</p>{match.matchedWords.length>0&&<p>Matched {match.matchedWords.length===1?"keyword":"keywords"}: <strong>{match.matchedWords.join(", ")}</strong></p>}{match.matchedWords.length>0&&<p className="fs-ranking-match-caution">A keyword match shows wording overlap only. It does not prove proficiency or replace evidence review.</p>}{skills.length>0?<div className="fs-ranking-skill-source"><p>Source: {sourceLabels[match.source]}</p><ul>{skills.map((skill,skillIndex)=>{const matched=containsMatchedWord(skill,match.matchedWords);return <li className={matched?"is-matched":undefined} key={`${skill}-${skillIndex}`}><span>{skill}</span>{matched&&<b>Matched keyword</b>}</li>;})}</ul></div>:match.excerpt?<><p>Source: {sourceLabels[match.source]||"Candidate clarification"}</p><blockquote>{match.excerpt}</blockquote></>:<p>No matching passage in the submitted application. This does not prove the candidate lacks the requirement.</p>}</section>;})}</details>
     </article>)}</div>
    </Panel>
