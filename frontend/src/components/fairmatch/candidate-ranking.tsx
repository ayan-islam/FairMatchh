...
export function CandidateRanking({jobs,auth,initialJobId}:{jobs:Job[];auth:string;initialJobId?:string}) {
 ...
 return <>
  <PageHeading eyebrow="EXPLAINED COMPARISON" title="Candidate ranking" description="Automatically compare submitted evidence for one job and hiring stage. Recruiters review the result before making decisions." actions={<Button variant="outline" onClick={refresh}>Refresh ranking</Button>}/>
  <div className="fm-notice"><div><strong>Automatic first-pass ranking is ready without scoring candidates by hand.</strong><p>FairMatch compares unique job-requirement terms with each submitted application, then orders candidates by weighted text match. A confirmed CV can help a candidate fill their application, but private CV files are never read by this employer ranking. A missing match does not mean a candidate lacks the skill. Check the cited passage before acting; this is not a qualification or success score.</p></div></div>
  ...
  {current&&<>
   <Panel className="fs-ranking-panel" title="Automatic evidence ranking" description="Generated whenever you open or refresh this page. No recruiter ratings are needed.">
    <p>{current.rubricCurrent?"Uses the current job rubric weights.":"Uses equal weights across the job requirements until a rubric is saved."} Repeated words count only once per passage. Matches are clues to inspect, not proof of ability.</p>
    {!current.requirements.length&&<p>Add clear job requirements in Jobs to enable automatic comparison.</p>}
    {current.requirements.length>0&&!current.autoRanked.length&&<p>No active applications in this stage yet.</p>}
    {current.autoRanked.map(row=><section className="fs-ranking-row" key={row.applicationId}>
     <div className="fs-ranking-row-heading"><div><h3>Rank {row.rank} · {row.applicationId}</h3><p>Text matches for {row.matched} of {row.total} requirements</p></div><strong>{row.score.toFixed(2)} / 100 match</strong></div>
     <details className="fs-ranking-details"><summary>Why this position?</summary>{row.matches.map((match,index)=><section key={index}><h3>{match.requirement} · {match.weight}% weight</h3><p>{match.matchedWords.length}/{match.totalTerms} distinct requirement terms found in one passage · {match.points.toFixed(2)} match points</p>{match.matchedWords.length>0&&<p>Matched terms: {match.matchedWords.join(", ")}</p>}{match.excerpt?<><p>Source: {sourceLabels[match.source]||"Candidate clarification"}</p><blockquote>{match.excerpt}</blockquote></>:<p>No matching passage in the submitted application. This is not evidence that the candidate lacks the requirement.</p>}</section>)}</details>
    </section>)}
   </Panel>
   <Panel className="fs-ranking-panel" title="Scoring rubric" description="Each requirement gets a weight and five defined rating levels. Weights must total 100%.">
    <p>{current.rubricCurrent?`Version ${current.rubric!.version} · Saved by ${current.rubric!.actor} · ${new Date(current.rubric!.at).toLocaleString()}`:current.rubric?"Job requirements changed. Update the rubric before new human assessments. Automatic matching uses equal weights meanwhile.":"No rubric saved yet. Automatic matching uses equal weights; save a rubric for custom weights and optional human assessments."}</p>
   ...
   <div className="fs-ranking-summary"><StatusBadge>{current.autoRanked.length} automatically compared</StatusBadge><StatusBadge>{current.ranked.length} recruiter assessments complete</StatusBadge><Button variant="outline" onClick={()=>exportCsv("fairmatch-ranking.csv",[["Job","Stage","Application","Automatic rank","Text match / 100","Requirements matched","Requirements total","Human rank","Human rubric score / 100","Human assessment status"],...current.autoRanked.map(r=>{const human=[...current.ranked,...current.pending].find(h=>h.applicationId===r.applicationId);return [current.jobTitle,stage,r.applicationId,String(r.rank),String(r.score),String(r.matched),String(r.total),human?.rank===null||!human?"":String(human.rank),human?.score===null||!human?"":String(human.score),human?.status||""];})])}>Export this comparison</Button></div>
   <h2 className="fs-ranking-section-title">Optional recruiter assessments</h2><p>The automatic order above needs no manual rating. Use the rubric below when you want a deeper, evidence-cited review of a candidate. Human scores are shown separately.</p>
