 ...
 return <>
  ...
  <section className="fs-ranking-toolbar" aria-label="Ranking controls">
   <div className="fs-ranking-filters">
    <Field label="Job"><select value={jobId} onChange={e=>{setJobId(e.target.value);setBoard(undefined);}}><option value="" disabled>Select a job</option>{jobs.map(j=><option key={j.id} value={j.id}>{j.title} · {j.status}</option>)}</select></Field>
    <Field label="Hiring stage"><select value={stage} onChange={e=>{setStage(e.target.value);setBoard(undefined);}}>{stages.map(s=><option key={s}>{s}</option>)}</select></Field>
   </div>
