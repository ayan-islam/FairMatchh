...
@SpringBootTest(properties={"fairmatch.seed=false","fairmatch.bootstrap.demo-enabled=true","fairmatch.bootstrap.admin-enabled=false","fairmatch.admin.password=FairMatchAdmin!2026","fairmatch.mail.dispatch=false"}) @AutoConfigureMockMvc
class PlatformFeaturesTest {
    ...
    @Test void jwtRolesAndAccountOwnershipAreEnforced() throws Exception {
        ...
        assertThat(username).startsWith("qa_");
        ...
        call(post("/api/public/auth/register"),null,Map.of("username","123456","contact","numeric-candidate@example.test","name","Invalid candidate username","password","TestPassword!123","role","CANDIDATE","organizationName",""),400);
    ...
    @Test void privatePdfStorageExtractionConfirmationAndDeletion() throws Exception {
        ...
        try {
            ...
            var highlights=Map.of("skills",List.of("Java","SQL"),"courses",List.of("Database Systems"),"projects",List.of("Built an inventory tracker with SQL"));
            call(post("/api/candidate/documents/"+id+"/confirmation"),token,Map.of("profile",profile,"highlights",highlights,"confirmed",false),400);
            call(post("/api/candidate/documents/"+id+"/confirmation"),token,Map.of("profile",profile,"highlights",Map.of("skills",List.of("Java"),"courses",List.of(),"projects",List.of("x".repeat(241))),"confirmed",true),400);
            call(post("/api/candidate/documents/"+id+"/confirmation"),token,Map.of("profile",profile,"highlights",highlights,"confirmed",true),200);
            ...
            assertThat(blind.toString()).contains(applicationId,"Database Systems","inventory tracker").doesNotContain("QA Candidate","@example.test",id);
