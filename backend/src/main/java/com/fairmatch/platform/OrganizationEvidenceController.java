...
class OrganizationEvidenceController {
    ...
    @GetMapping("/api/employer/organization/evidence") OrganizationEvidenceService.Bundle own(Principal p){return evidence.bundle(platform.requireOrganizationOwner(p.getName()));}
    ...
    OrganizationEvidenceService.Evidence upload(@RequestParam MultipartFile file,@RequestParam String type,@RequestParam String description,@RequestParam long expectedVersion,Principal p)throws java.io.IOException {
        return evidence.upload(platform.requireOrganizationOwner(p.getName()),type,description,file.getOriginalFilename(),file.getBytes(),expectedVersion,p.getName());
    }
    @DeleteMapping("/api/employer/organization/evidence/{id}") Map<String,Boolean> remove(@PathVariable String id,@Valid @RequestBody Removal input,Principal p){evidence.remove(platform.requireOrganizationOwner(p.getName()),id,input.expectedVersion(),p.getName());return Map.of("removed",true);}
    @GetMapping("/api/employer/organization/evidence/{id}/file") ResponseEntity<byte[]> ownFile(@PathVariable String id,Principal p){return download(platform.requireOrganizationOwner(p.getName()),id,p.getName());}
